...
if(empty($email_data)){
	...
	$date	= date('ymd');
	$hasil = mysqli_query($conn, "SELECT max(id_user) as maxKode FROM user");
...
//	 $mail->Timeout = 3600;
	...
	$mail->setFrom('pengaduanmobile@gmail.com', 'Aplikasi Pengaduan Masyarakat');
	$mail->addAddress($email, $nama);
	$mail->isHTML(true);

	$mail->Subject = 'Info Akun Aplikasi Pengaduan Masyarakat';
	...
	<h2 style="margin: 0; font-size: 30px; font-weight: 700; font-color: #FFFFFF; letter-spacing: -1px; line-height: 40px;">Pengaduan Masyarakat Mobile</h2>
	</td>
	</tr>
	</table>
	</td>
	...
	<p style="margin: 0;">Silahkan login ke aplikasi Pengaduan Masyarakat Kabupaten Temanggung dengan username dan password di bawah</p>
	</td>
	</tr>
	...
	if(!$mail->send()) {
		...
		$query = mysqli_query($conn, "INSERT INTO user (id_user, nama, email, tgl_daftar, status_aktif, username, pass, status_user) VALUES ('$kdso', '$nama', '$email', CURRENT_DATE, 'Y', '$username','$pass','01') ");
		if ($query) {
